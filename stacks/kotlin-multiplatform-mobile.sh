#!/bin/bash
# Kotlin Multiplatform Mobile (KMM) with Jetpack Compose Stack Detection
# Contributed via claude-ally automated contribution system

detect_kotlin_multiplatform_mobile() {
    local project_dir="$1"
    local confidence=0
    local tech_stack="Kotlin Multiplatform Mobile (KMM) with Jetpack Compose"
    local project_type="mobile-app"
    ...
    if [[ $confidence -ge 50 ]]; then
        echo "kotlin-multiplatform-mobile|$tech_stack|$project_type|$confidence"
        return 0
    fi
...
get_kotlin_multiplatform_mobile_patterns() {
    cat << 'EOL'
${tech_stack^^} PATTERNS
